...
provide(isInsideTemplate, isTemplate)
provide(parentGroup, group)
...
<template>
  <div v-if="group.blocks.length" class="w-auto">
    <EditorBlockRender
      ...
      :key="block"
      :block="block"
      @start-dragging="startDragChild($event, index)"
      @attach-changed="(change) => attachmentChanged(index, ...change)"
