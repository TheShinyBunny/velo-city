...
const group: BlockGroup | undefined = inject(parentGroup, undefined)
...
<template>
  <UContextMenu :model-value="open" :virtual-element="contextMenu.element" @mousedown.left.stop @update:model-value="$emit('update:open', $event)">
    <div class="p-1">
      <button
        v-for="item of items"
        class="group flex items-center gap-2 w-full px-2 py-1.5 text-sm rounded-md text-gray-700
            dark:text-gray-200 hover:bg-gray-100 hover:dark:bg-gray-900 hover:text-gray-900 hover:dark:text-white"
